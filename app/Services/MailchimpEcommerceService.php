...
class MailchimpEcommerceService
{
    ...
    /**
     * Add or update a completed local order in Mailchimp's e-commerce store.
    ...
    public function syncOrder(Order $order): array
    {
        ...
        try {
            ...
            // If this very checkout included explicit newsletter consent,
            // create the audience member first with status_if_new semantics.
            // The following transactional customer upsert can then stay false
            // without losing new consent or re-subscribing an unsubscribe.
            $consentResult = $this->syncCheckoutNewsletterConsent($order, $email);
            if (! $consentResult['ok'] && ! empty($consentResult['stop'])) {
    ...
    private function ensureStore(): array
    {
        ...
        $storePayload = [
            ...
            'name' => $this->storeName(),
            'currency_code' => $this->currencyCode(),
            // Safe default: keep Mailchimp order notifications and other store
            // automations paused until they are explicitly enabled in config.
    ...
    /** @return array{ok:bool,error:?string,stop?:bool} */
    private function syncCheckoutNewsletterConsent(Order $order, string $email): array
    {
        try {
            ...
            $subscriberQuery = NewsletterSubscriber::query()
                ->where('email', $email)
            ...
            if (Schema::hasColumn('newsletter_subscribers', 'mailchimp_synced_at')) {
                $subscriberQuery->whereNull('mailchimp_synced_at');
            }
            ...
            $result = app(MailchimpNewsletterService::class)->syncSubscriber($subscriber);

            if ($result['ok'] && Schema::hasColumn('newsletter_subscribers', 'mailchimp_synced_at')) {
            ...
            // Consent sync is best-effort and never blocks purchase reporting.
            return ['ok' => true, 'error' => null];
        }
    }
    ...
    {
        return trim((string) config('services.mailchimp.ecommerce_store_id', 'antikvarijat-biblos'));
    }

    private function storeName(): string
    {
        return trim((string) config('services.mailchimp.ecommerce_store_name', 'Antikvarijat Biblos'))
            ?: 'Antikvarijat Biblos';
    }
