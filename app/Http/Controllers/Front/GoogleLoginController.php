...
class GoogleLoginController extends Controller
{
    ...
    public function redirect(
        ...
        if (! $settings->enabled()) {
            return $this->fail($request, __('front.auth.google_unavailable'));
        }
        ...
        try {
            ...
            return $this->fail(
                $request,
                __('front.auth.google_unavailable'),
    ...
    public function callback(
        ...
        if (! $this->validTransaction($transaction)) {
            return $this->fail($request, __('front.auth.google_expired'));
        }
        ...
        if ($googleError !== '') {
            if ($googleError === 'access_denied') {
                return $this->fail($request, __('front.auth.google_cancelled'), '', $transaction['redirect'] ?? null);
            }
        ...
        if (! $this->authoritativeGoogleEmail($email, $claims)) {
            return $this->fail(
                $request,
                __('front.auth.google_verified_email'),
                '',
                $transaction['redirect'] ?? null
            );
        }
        ...
        if (! $user || ! $user->details) {
            return $this->fail(
                $request,
                __('front.auth.google_account_missing'),
                '',
                $transaction['redirect'] ?? null
            );
        }
        ...
        if (! $user->details->status) {
            return $this->fail(
                $request,
                __('front.auth.account_inactive'),
                '',
                $transaction['redirect'] ?? null
            );
        }
    ...
    private function fail(
        ...
        $target = $this->safeRedirect($request, $redirect)
            ?: \App\Helpers\LocaleHelper::route('index');
