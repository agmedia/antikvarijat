<form id="payForm" name="pay" class="w-100 needs-validation" action="{{ $data['action'] }}" novalidate method="POST">
    ...
    <div class="d-flex mt-3">
        ...
        <div class="w-50 ps-2"><button id="paySubmit" class="btn checkout-cta d-block w-100" type="submit"><span class="d-none d-sm-inline">{{ __('front.checkout.complete_order') }}</span><span class="d-inline d-sm-none">{{ __('front.checkout.complete_purchase') }}</span><i class="fa-solid fa-arrow-right mt-sm-0 ms-1"></i></button></div>
    </div>
